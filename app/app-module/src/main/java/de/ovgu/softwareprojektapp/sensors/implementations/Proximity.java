...
/**
 * {@link DataSource} feeding {@link Sensor#TYPE_PROXIMITY} data
 */
public class Proximity extends AbstractSensor {
    /**
     * Create an{@link Proximity} sensor; does not start anything yet, use {@link #start()} or {@link #setRunning(boolean)}
     * to start receiving events
     *
     * @param context android system context needed for sensors
     */
    public Proximity(Context context) {
        super(context, Sensor.TYPE_PROXIMITY, SensorType.Proximity);
    }

    /**
     * Static sensor speed definition for all{@link Proximity} instances. defaults to sensor_delay_game.
     */
    private static SetSensorSpeed.SensorSpeed mSensorSpeed = SetSensorSpeed.SensorSpeed.SENSOR_DELAY_GAME;

    /**
     * update the sensor speed for all{@link Proximity} instances
     *
    ...
    /**
     * Get the sensor speed currently set for all {@link Proximity} instances
     */
    @Override
